...
# while문 안에다가 계속 탐색하는 rfind()같은 거 넣으면 시간초과: O(MN)
    ...
    print (f"#{test_case} {answer}")
    '''```
코드 중에 이 부분이 있죠? `if t[shift+p_end] not in p_dict.keys():`

파이썬에서 딕셔너리에 키가 있는지 확인할 때는 `.keys()`를 빼고 그냥 **`if t[...] not in p_dict:`**라고 쓰는 게 훨씬 빠르고 더 파이썬다운 방식입니다. `.keys()`를 쓰면 별도의 리스트(혹은 뷰)를 거쳐야 하지만, 그냥 쓰면 해시 테이블을 직접 조회하기 때문에 더 빠릅니다.